...
@section('title', 'Manage Reports - Admin - ImageBoard')
...
<h2 style="color: #AF0A0F; margin-bottom: 20px;">Post Reports</h2>

<div style="background: #F0E0D6; padding: 15px; border: 1px solid #D9BFB7; margin-bottom: 20px;">
    <h3 style="margin-top: 0;">Filters</h3>
    <form action="{{ route('admin.reports.index') }}" method="GET" style="display: flex; gap: 10px; flex-wrap: wrap; align-items: end;">
        <div>
        ...
        <div>
            ...
            <a href="{{ route('admin.reports.index') }}" style="display: inline-block; padding: 5px 15px; background: #EEE; border: 1px solid #CCC; text-decoration: none; color: #000;">Clear</a>
        </div>
    </form>
</div>
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                @if($report->status === 'pending')
                    <form action="{{ route('admin.reports.review', $report) }}" method="POST" style="display: inline;">
                        @csrf
